...
class macOS:
    ...
    def start(self) -> int:
        ...
        if result != 0:
            ...
            return 502
        command = "brew install jsoncpp sqlite3 sqlite-utils fmt clang-format curl googletest gcc zlib cmake libzip openssl wget boost asio libssh2 unzip zip postgresql libpq"
        result = os.system(command)
        command = "wget https://github.com/madler/zlib/archive/refs/tags/v1.1.2.zip && unzip v1.1.2.zip && cd zlib-1.1.2 && sudo ./configure && sudo make && sudo make install"
        result = os.system(command)
